...
//*****************************************************************************
// 2x2 inverse, identity if impossible
//*****************************************************************************
Matrix2& Matrix2::invert()
{
    float determinant = m[0] * m[3] - m[1] * m[2];
    if (std::abs(determinant) <= MATRIX_EPSILON)
    {
        return identity();
    }
...
//*****************************************************************************
// 3x3 inverse, identity if impossible
//*****************************************************************************
...
{
    // if the 4th row is [0,0,0,1] then it's an affine matrix and has no projective transform
...
Matrix4& Matrix4::invertProjective()
{
    ...
    // NOTE: this function assumes det(A) is already checked. if |A|=0 then it's impossible
...
//*****************************************************************************
// 4x4 inverse using Cramer's Rule, identity if impossible
// M^-1 = adj(M) / det(M)
//*****************************************************************************
...
//*****************************************************************************
// 4x4 rotation matrix, multiply with itself
//*****************************************************************************
...
{
    float c = cosf(deg2rad(angle)); // cosine
    float s = sinf(deg2rad(angle)); // sine
    float xx = x * x;
    float xy = x * y;
    float xz = x * z;
    float yy = y * y;
